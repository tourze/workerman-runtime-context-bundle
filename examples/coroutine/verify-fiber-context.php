...
$worker->onMessage = function (TcpConnection $connection, $data) use (&$results): void {
    ...
    // 创建子协程并验证上下文隔离
    Coroutine::create(function () use ($connection, $requestId, &$contextInfo): void {
        ...
        Coroutine::create(function () use ($connection, $requestId, &$contextInfo): void {
            ...
            for ($i = 1; $i < count($fibers); ++$i) {
                if (isset($fibers[$i]['originalValue']) && null !== $fibers[$i]['originalValue']) {
                    $context_isolation = false;
                    cecho('警告：协程上下文未正确隔离');
                }
            }
...
$worker->onWorkerStart = function (): void {
    ...
    // 延迟2秒后直接触发测试，不依赖外部命令
    Timer::add(2, function () use (&$worker): void {
        cecho('自动触发测试...');
